...
app.put('/devices/:id', (req, res)=> {
	...
	let qrr = `update devices set consoleip='${consoleip}', consoleport='${consoleport}', managementip='${managementip}', powercycleip='${powercycleip}', powercycleport='${powercycleport}', teamname='${teamname}', projectname='${projectname}', serialnumber='${serialnumber}', TG='${tg}' where id=${dId}`;
	...
	if(valid && !redundency){
		...
		if(i==ans.length-1){
			if(deviceArray.length>0){
				...
				db.query(qrr, [deviceArray], (err, results)=> {
				if(err){
					 res.status(663).json({
						message: 'wrong file chosen'
					})
